...
class DockerSwarmProvisioner(ContainerProvisioner):
    ...
    def _get_service(self) -> Service:
        ...
        if num_services != 1:
            if num_services > 1:
                raise RuntimeError(f"{self.__class__.__name__}: Found more than one service "
    ...
    async def get_container_status(self, iteration: Optional[str]) -> str:
        ...
        if iteration:  # only log if iteration is not None (otherwise poll() is too noisy)
            self.log.debug(f"{iteration}: Waiting to connect to docker container. "
    ...
    async def terminate_container_resources(self, restart: bool = False) -> bool | None:
        ...
        if service:
            try:
                ...
                self.log.debug(f"{self.__class__.__name__} Termination of service: "
        ...
        if result:
            self.log.debug(f"{self.__class__.__name__}.terminate_container_resources, "
                           f"service {self.container_name}, "
                           f"kernel ID: {self.kernel_id} has been terminated.")
            self.container_name = None
            result = None  # maintain jupyter contract
        else:
            self.log.warning(f"{self.__class__.__name__}.terminate_container_resources, "
...
class DockerProvisioner(ContainerProvisioner):
    ...
    def _get_container(self) -> Container:
        ...
        if num_containers != 1:
            if num_containers > 1:
                raise RuntimeError(f"{self.__class__.__name__}: Found more than one container "
            ...
            if container.status:
                ...
                if container_status == 'running' and self.assigned_host == '':
                    ...
                    if len(networks) > 0:
                        ...
                        self.log.debug(f"Using assigned_ip {self.assigned_ip} from docker network '{docker_network}'.")
                    else:
                        self.log.warning(f"Docker network '{docker_network}' could not be located in "
    ...
    async def terminate_container_resources(self, restart: bool = False) -> None:
        ...
        if container:
            try:
                ...
                self.log.debug(f"Container termination for container: {container.name} raised exception: {err}")
                if isinstance(err, NotFound):
                    pass  # okay if its not found
                else:
                    result = False
