...
def test_invalid_parameters(tmpdir):
    ...
    with pytest.raises(ValueError) as ve:
        assert spec_utils.copy_kernelspec_files(
            spec_dir, launcher_type="python", resource_type="bogus-type"
        )
    ...
    assert str(ve.value).startswith(
        "Invalid parameters.  At least one of 'launcher_type' or 'resource_type' must"
    )
...
def test_copy_r_launcher(tmpdir):
    ...
    assert path.isfile(launcher_file)
    server_listener_file = path.join(spec_dir, "scripts", "server_listener.py")
    assert path.isfile(server_listener_file)

    spec_utils.delete_staging_directory(staging_dir)
    assert not path.exists(server_listener_file)
    assert not path.exists(launcher_file)
    assert not path.exists(scripts_dir)
