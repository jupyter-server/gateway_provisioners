...
class SshSpecApp(BaseSpecApp):
    ...
    description = "Creates a Jupyter kernel specification for use within a cluster of hosts via SSH."
...
each be specified via separate options: --remote-hosts host1 --remote-hosts host2""")
    ...
    spark_master = Unicode(default_value=spark_master_default_value, config=True,
...
class SshProvisionerApp(Application):
    ...
    subcommands = dict({'install': (SshSpecApp, SshSpecApp.description.splitlines()[0]), })
    aliases = {}
    flags = {}
